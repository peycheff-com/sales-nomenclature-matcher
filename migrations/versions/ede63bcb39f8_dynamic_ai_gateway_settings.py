...
down_revision: str | Sequence[str] | None = "0004_must_change_password"
...
def upgrade() -> None:
    ...
    conn.execute(sa.text("DELETE FROM system_settings WHERE key = ANY(:keys)"), {"keys": old_keys})
...
def downgrade() -> None:
    ...
    conn = op.get_bind()
    res = conn.execute(
        sa.text("SELECT value FROM system_settings WHERE key = 'providers_registry'")
    )
    row = res.fetchone()
    if row:
        try:
